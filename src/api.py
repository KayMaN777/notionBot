...
async def find_project_name(token: str, name: str) -> int:
    api = TodoistAPIAsync(token)
    try:
        projects = await api.get_projects()
        for project in projects:
...
async def get_task_id(token: str, name: str, content: str) -> str | None:
    api = TodoistAPIAsync(token)
    project_id = await get_project_id(token, name)
    try:
...
async def find_task_content(token: str, name: str, content: str) -> int:
    api = TodoistAPIAsync(token)
    project_id = await get_project_id(token, name)
    try:
        tasks = await api.get_tasks(project_id=project_id)
...
async def update_task(
    ...
    try:
        if new_content is not None:
            await api.update_task(task_id=task_id, content=new_content)
        if new_description is not None:
            await api.update_task(task_id=task_id, description=new_description)
        if new_due_string is not None:
            await api.update_task(task_id=task_id, due_string=new_due_string)
        return True
    except Exception as error:
        print(error)
        return False
