...
class MyApi:
    ...
    def AddProject(self, name, parent = None, style = "list", color = "charcoal"):
        try:
            ...
            self.api.add_project(name = name, parent_id = parent_id, view_style = style, color = color)
        except Exception as e:
            raise e
    ...
    def RenameProject(self, name, new_name):
        try:
            project_id = self.GetProjectId(name)
            self.api.update_project(project_id = project_id, name = new_name)
        except Exception as e:
            raise e
    ...
    def AddTask(self, content, project = "Inbox", description = None, date = None, priority = 1):
        try:
            project_id = self.GetProjectId(project)
            self.api.add_task(content=content, project_id = project_id, description = description, due_date = date, priority = priority)
        except Exception as e:
            raise e
